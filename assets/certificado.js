...
const ZONA_TEXTO = { topo: 0.30, base: 0.75, esquerda: 0.15, direita: 0.85 };
...
const CORES_TRILHA = {
  "participantes": { principal: [63, 107, 74], dourado: [201, 154, 62] },
...
function montarPartesTexto(trilha, registro) {
  ...
  if (trilha === "avaliadores") {
    ...
    return [
      { t: `Certificamos que `, b: false },
      { t: nomes, b: true },
      { t: ` participou(aram) da ação Missão Rosa${registro.detalhe ? " " + registro.detalhe : ""}, realizada durante o ${NOME_EVENTO}, ${DATA_EVENTO}, no ${LOCAL_EVENTO}.`, b: false }
    ];
  } else {
    return [
...
async function gerarCertificadoPNG(trilha, registro) {
  ...
  const larguraMax = canvas.width * (ZONA_TEXTO.direita - ZONA_TEXTO.esquerda);
